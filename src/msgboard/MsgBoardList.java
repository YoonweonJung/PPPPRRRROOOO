...
public class MsgBoardList implements ShopreboardSuper{

	@Override
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		System.out.println("msgBoardList컨트롤러진입-execute()");
		//할일
		//1. 파라미터받기
	    // nowPage = (선택)보고싶은페이지
		...
		//총게시물수 조회
		String sql1 = boardSQL.getSQL(boardSQL.SELECT_MSGCNT);
		...
		try {
			...
			System.out.println("(원글전체)목록보기 sql="+sql1);
		...
		ArrayList<MsgVO> list = new ArrayList<MsgVO>(); //4번째 모델방식에 필요한 ArrayList선언
		...
		try {
			...
			//while( rs.next() ) {   
				...
				vo.setMb_stat(rs.getInt("mb_comf"));
				...
				System.out.println(vo.getM_id()+"/"+vo.getMb_no()+"/"+vo.getMb_title()+"/"
									+vo.getMb_idchk()+"/"+vo.getM_nick()+"/"+vo.getMb_date());
			}
		...
		return "../view/reboard/msgboardList.jsp";
	}
